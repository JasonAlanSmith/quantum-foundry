...
combining:
- Quantum Foundry defaults
- A Quantum Foundry configuration file (creating one on first run
  if needed)
...
from qf.utils.random import create_rng

logger = logging.getLogger("qf")


def boot_app_from_ns(ns: Namespace) -> AppContext:
    cr = ns.config_dir or qf_config_root
    dr = ns.data_dir or qf_data_root
    cp = ns.config_path or qf_config_path
    log_file = ns.log_file or qf_logs_dir
    seed = ns.seed
    dcd = qf_config
    ...
    if not app_config_file_exists(cp):
        store_qf_config(Path(cp), dcd)

    cd = load_qf_config(Path(cp))

    default_seed = "rng-global-default-seed-v0.1.0"
    seed_value = seed or (
        cd.get("seed", default_seed) if cd else default_seed
    )
    ...
    return AppContext(
        ...
        projects_root=Path(
            cd.get("project", {}).get(
                "projects_root", Path(dr) / "projects"
            )
        ),
        max_generation=cd.get("limits", {}).get(
...
def validate_app_dirs(app_config_root: str, app_data_root: str) -> None:
    if not config_dir_exists(app_config_root):
        make_qf_config_dir(app_config_root)

    if not data_dir_exists(app_data_root):
        make_qf_data_dir(app_data_root)
...
def setup_logger(ns: Namespace) -> None:
    ...
    Rules for --log-file:
      ...
      - existing directory path: auto-generate a file name in that
        directory
    ...
    if raw:
        ...
        if raw_str in {".", "./", ".\\"}:
            ...
            # If they provided a directory, write an auto-named log
            # file inside it.
            if p.exists() and p.is_dir():
                log_path = p / Path(generate_log_file_name()).name
                log_file = str(log_path)
            else:
                # Otherwise treat it as a file path (even if it doesn't
                # exist yet).
                log_file = str(p)
