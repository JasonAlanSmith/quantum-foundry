...
This module formats generated values into SQL INSERT statements
efficiently. It contains fast value-formatting helpers used by the
hot generation loop.
"""
...
from qf.emitters.base import Emitter
...
def _format_bool(value: Any) -> str:
    ...
    if isinstance(value, str):
        v = value.strip().lower()
        return (
            "true" if v in {"true", "t", "1", "yes", "y"} else "false"
        )
...
def get_formatter(sql_type: SqlType) -> Callable[[Any], str] | None:
    """
    Return the SQL literal formatter for a given SqlType, if
    registered.
    """
...
class SqlEmitter(Emitter):
    ...
    def emit(
        self, table: str, emit_defs: list[ColumnEmitDefinition]
    ) -> Iterable[str]:
...
def format_values_fast(
    ...
    """
    Format a row of values using a parallel list of pre-resolved
    formatters.
    """
