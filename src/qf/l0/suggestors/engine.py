...
from qf.l0.suggestors.fallback import type_based_fallback
...
def choose_best(
    ...
    # Sort by: confidence desc, priority desc, then stable (original
    # order preserved by sort stability if equal)
    suggestions.sort(
        key=lambda s: (s.confidence, s.priority), reverse=True
    )
