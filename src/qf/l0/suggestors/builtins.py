...
from qf.l0.suggestors.base import PatternSuggestor
...
class EmailSuggestor(PatternSuggestor):
    ...
    def suggest(
        self, col: ColumnInspection
    ) -> PatternSuggestion | None:
        n = col.name.lower()
        if "email" in n or re.search(
            r"\b(e_mail|emailaddr|email_address)\b", n
        ):
            return _mk(
                strategy="email",
                pattern_id="email",
...
class UuidSuggestor(PatternSuggestor):
    ...
    def suggest(
        ...
        # Type-based clue: PostgreSQL UUID type often stringifies to
        # "UUID"
        if t == "UUID":
            return _mk(
                strategy="uuid",
...
class TimestampSuggestor(PatternSuggestor):
    ...
    def suggest(
        ...
        if any(
            ...
            reason1 = "column name suggests datetime; "
            reason2 = "type not obviously datetime"
...
class StatusCodeSuggestor(PatternSuggestor):
    ...
    def suggest(
        ...
        if not any(
            k in n
            for k in ("status", "state", "type", "category", "code")
        ):
            return None
        ...
        if any(
            ...
            reason1 = "name suggests status/type/code; "
            reason2 = "use a choice pool"
            reason = reason1 + reason2
            return _mk(
...
class ForeignKeySuggestor(PatternSuggestor):
    ...
    def suggest(
        ...
        # Example: strategy indicates we should draw values from
        # referenced table
        ref_table = None
        if col.foreign_key:
