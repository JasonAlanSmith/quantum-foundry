...
class TextGenerator(ValueGenerator):
    ...
    def __init__(
        ...
        # PERF: Precompute valid choices once; do not copy/mutate
        # choices per row.
        self._valid_choices: list[str] = [
            c for c in choices if min_len <= len(c) <= max_len
        ]

        if not self._valid_choices and not field_def.allow_null:
            # Fallback: generate a random string if no valid choices
            # and field cannot be null
    ...
    def generate(self) -> str | None:
        """
        Return a random valid choice, or None when nulls are
        allowed.
        """
