...
def load_qf_config(config_path: Path) -> dict[str, Any]:
    path = FilePathValidator(config_path).validate_path()
    with path.open("rb") as f:
        data: dict[str, Any] = tomllib.load(f)
        config = data
    validate_qf_config(config)
    return config
...
def store_qf_config(config_path: Path, config: dict[str, Any]) -> None:
    validate_qf_config(config)
...
def make_qf_data_dir(path: str) -> None:
    p = Path(path)
    if not p.exists():
