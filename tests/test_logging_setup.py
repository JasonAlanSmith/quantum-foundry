...
def test_setup_logging_console_only_adds_stderr_handler(
    qf_logger: logging.Logger,
) -> None:
...
def test_setup_logging_idempotent_when_force_false(
    qf_logger: logging.Logger,
) -> None:
    ...
    assert (
        first == second
    ), "Expected no duplicate handlers when already configured"
...
def test_setup_logging_writes_csv_and_jsonl(
    tmp_log_base: Path, qf_logger: logging.Logger
) -> None:
