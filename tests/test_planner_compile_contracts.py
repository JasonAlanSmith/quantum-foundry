...
def test_boolean_planner_compile_returns_callable(
    ...
    sql_type, gen = BooleanPlanner().compile(
        col_short_string, sug, plan_context
    )
    v = gen()
...
def test_int_planner_compile_returns_callable(
    plan_context: PlanContext, col_short_string: ColumnInspection
) -> None:
    sug = PatternSuggestion(
        strategy="int_range",
    ...
    sql_type, gen = IntPlanner().compile(
        col_short_string, sug, plan_context
    )
    v = gen()
...
def test_uuid_planner_compile_returns_uuid_like_string(
    ...
    sql_type, gen = UUIDPlanner().compile(
        col_short_string, sug, plan_context
    )
    v = gen()
    assert v is not None
...
def test_datetime_planner_compile_returns_iso_like_string(
    ...
    sql_type, gen = DateTimePlanner().compile(
        col_short_string, sug, plan_context
    )
    v = gen()
    assert v is not None
