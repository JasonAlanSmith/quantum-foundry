...
def test_suggest_for_column_selects_first_name(
    col_first_name: ColumnInspection,
) -> None:
    ...
    # planner strategy stays generic
    assert suggestion.strategy == "string"
    assert suggestion.pattern_id == "first_names"
    assert suggestion.suggestor == "first_name"
    assert suggestion.confidence >= CONFIDENCE


def test_suggest_for_column_falls_back_when_no_suggestors(
    col_short_string: ColumnInspection,
) -> None:
