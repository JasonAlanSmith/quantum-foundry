...
def test_emitter_renders_null_without_quotes() -> None:
    ...
    cols = [
        ColumnEmitDefinition(
            name="note", data_type=SqlType.STRING, value=None
        )
    ]
    sql = e.emit("t1", cols)
...
def test_emitter_renders_boolean_as_true_false() -> None:
    e = SqlEmitter()
    sql_true = e.emit(
        "t1", [ColumnEmitDefinition("is_active", SqlType.BOOLEAN, True)]
    )
    sql_false = e.emit(
        "t1",
        [ColumnEmitDefinition("is_active", SqlType.BOOLEAN, False)],
    )
...
def test_emitter_renders_numeric_without_quotes() -> None:
    e = SqlEmitter()
    sql = e.emit(
        "t1", [ColumnEmitDefinition("amount", SqlType.DECIMAL, 12.34)]
    )
    sql_str = next(iter(sql))
