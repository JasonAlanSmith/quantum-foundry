...
def test_text_generator_filters_choices_by_length() -> None:
    ...
    field = TextFieldDefinition(
        allow_null=False, fixed_length=None, min_length=2, max_length=4
    )
...
def test_text_generator_falls_back_when_no_valid_and_not_nullable() -> (
    None
):
    rng = random.Random(1)
    choices = ["a", "b"]
    field = TextFieldDefinition(
        allow_null=False, fixed_length=None, min_length=3, max_length=4
    )
...
def test_text_generator_returns_none_when_no_valid_and_nullable() -> (
    None
):
    rng = random.Random(1)
    choices = ["a", "b"]
    field = TextFieldDefinition(
        allow_null=True, fixed_length=None, min_length=3, max_length=4
    )
    gen = TextGenerator(rng, choices, field)
